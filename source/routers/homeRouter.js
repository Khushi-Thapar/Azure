    ...
    if(password === cpassword ){
         ...
         userData.save(err=>{
             if(err){
                ...
                res.render('RegisterLogin',{title :'Done',password:'',email:''})
             }
         })
       
    const useremail = await homeSchema.findOne({email:email});
     if(email === useremail.email){
        res.render('registermain',{title :'',password:'',email:'Email is Already there plz chose different one'})
     }else{
         console.log('err')
     }

    }else{
        res.render('registermain',{title :'',password:'Password not Matching',email:''})
    }
   }catch(error){

    res.render('registermain',{title :'Error in Code',password:'',email:''})
   }
})
